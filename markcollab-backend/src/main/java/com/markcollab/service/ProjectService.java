...
public class ProjectService {
    ...
    // NOVO MÉTODO para buscar a entidade Project completa (não DTO)
    // Usado pelo ProjectController para o pagamento
    public Project findProjectById(Long projectId) {
        return projectRepository.findById(projectId)
                .orElseThrow(() -> new RuntimeException("Project not found with ID: " + projectId));
    }

    // NOVO MÉTODO para buscar a entidade Employer completa (não DTO)
    // Usado pelo ProjectController para o pagamento
    public Employer findEmployerByCpf(String employerCpf) {
        return employerRepository.findById(employerCpf) // Supondo que o ID do Employer seja o CPF
                .orElseThrow(() -> new RuntimeException("Employer not found with CPF: " + employerCpf));
    }

    // Se seu EmployerRepository.findById() espera um Long, e não String, você precisará ajustar
    // Se o CPF é a chave primária String, então está correto.
}
